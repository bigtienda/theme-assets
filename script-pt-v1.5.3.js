...
let typebotContainer = document.querySelector('typebot-standard').shadowRoot.querySelector('.typebot-container');

let topBar = document.createElement('div');
topBar.id = 'top-bar';
topBar.classList.add('absolute', 'z-20', 'top-0', 'flex', 'h-[70px]', 'w-screen', 'justify-between', 'border-b', 'bg-[#008069]', 'px-2', 'py-3', 'dark:border-[#8696a00a]', 'dark:bg-[#202c33]', 'dark:shadow-[0_1px_3px_rgba(11,20,26,0.4)]');

let leftArea = document.createElement('div');
leftArea.id = 'left-area';
leftArea.classList.add('flex', 'items-center');

let backButton = document.createElement('div');
backButton.id = 'back-button';
backButton.classList.add('h-7', 'w-7', 'cursor-pointer');

let backButtonSvg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
backButtonSvg.setAttribute('xmlns', 'http://www.w3.org/2000/svg');
backButtonSvg.setAttribute('viewBox', '0 0 72 72');
...
profileImage.classList.add('cursor-pointer');

let profileImg = document.createElement('img');
profileImg.src = profilePhotoUrl;
profileImg.classList.add('h-9', 'w-9', 'rounded-full');

profileImage.appendChild(profileImg);

let textProfile = document.createElement('div');
textProfile.id = 'text-profile';
textProfile.classList.add('ml-3', 'flex', 'flex-col', 'gap-2', 'leading-none', 'text-white', 'dark:text-[#e9edef]', 'relative');

let name = document.createElement('div');
name.id = 'name';
name.classList.add('cursor-pointer', 'text-lg', 'font-semibold', 'leading-none');
name.textContent = profileName;

let status = document.createElement('div');
status.id = 'status';
status.classList.add('text-sm', 'font-normal', 'leading-none');
status.textContent = 'online';

textProfile.appendChild(name);
textProfile.appendChild(status);
...
rightArea.appendChild(moreIcon);
...
/* begin:: top-message */
let chatViewContainer = document.querySelector('typebot-standard').shadowRoot.querySelector('.typebot-container .typebot-chat-view');

let chatStartInfo = document.createElement('div');
chatStartInfo.classList.add('chat-start-info', 'w-full', 'flex', 'flex-col', 'items-center', 'px-5', 'py-5', 'gap-4');

let dateInfo = document.createElement('div');
dateInfo.classList.add('bg-[#f8f7f7]', 'dark:bg-[#1c272d]', 'py-1', 'px-3', 'rounded-md', 'w-min', 'text-[#54656f]', 'dark:text-[#8696a0]', 'shadow-[0_1px_0.5px_rgba(11,20,26,0.08)]');
dateInfo.textContent = 'Hoje';

chatStartInfo.appendChild(dateInfo);

let securityMessage = document.createElement('div');
securityMessage.classList.add('py-1.5', 'px-3', 'rounded-md', 'bg-[#cbf2ee]', 'dark:bg-[#1c272d]', 'p-4', 'text-center', 'leading-tight', 'shadow-[0_1px_0.5px_rgba(11,20,26,0.08)]');

let iconSpan = document.createElement('span');
iconSpan.classList.add('inline-flex', 'w-3.5', 'h-3.5', 'align-top', 'mt-1');

let iconSvg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
iconSvg.setAttribute('xmlns', 'http://www.w3.org/2000/svg');
iconSvg.setAttribute('viewBox', '0 0 11 11');
iconSvg.innerHTML = '<path d="M5 0a5 5 0 1 1 0 10A5 5 0 0 1 5 0Zm-.027 4.118a.649.649 0 0 0-.649.648v2.757a.649.649 0 0 0 1.298 0V4.766a.649.649 0 0 0-.649-.648ZM5 1.892c-.227 0-.406.069-.538.206a.73.73 0 0 0-.197.527c0 .216.067.393.202.53A.712.712 0 0 0 5 3.363c.22 0 .399-.068.533-.206a.728.728 0 0 0 .202-.531.726.726 0 0 0-.2-.527c-.133-.137-.311-.206-.535-.206Z" class="dark:fill-[#5faca7] fill=#54656f"/>';

iconSpan.appendChild(iconSvg);

let messageSpan = document.createElement('span');
messageSpan.classList.add('text-[#54656f]', 'dark:text-[#5faca7]', 'align-top', 'text-sm');
messageSpan.textContent = 'Esta empresa usa um serviço seguro da empresa Meta para gerenciar esta conversa. Clique para saber mais.';

securityMessage.appendChild(iconSpan);
securityMessage.appendChild(messageSpan);

chatStartInfo.appendChild(securityMessage);

chatViewContainer.insertAdjacentElement('afterbegin', chatStartInfo);
/* end: top-message */
    ...
    if (allMessages.length > messagesLength) {
      for (let i = messagesLength; i < allMessages.length; i++) {
        ...
        if (figureMessage) {
          ...
          timeContainer.classList.add('absolute', 'bottom-2.5', 'right-4', 'inline-flex', 'items-center', 'gap-1');
          let timeText = document.createElement('div');
          timeText.classList.add('text-xs', 'text-[rgba(233,237,239,.9)]');
          timeText.textContent = messageTime;
          timeContainer.appendChild(timeText);

          let backgroundSpan = document.createElement('span');
          backgroundSpan.classList.add('absolute', 'inline-flex', 'h-10', 'w-[calc(100%-16px)]', 'bg-gradient-to-t', 'from-[rgba(11,20,26,.5)]', 'to-[rgba(11,20,26,0)]', 'bottom-2', 'items-center', 'flex', 'p-2', 'rounded-b-md');
          ...
          if (messageAudio) {
            ...
            let timeContainer = document.createElement('span');
            timeContainer.classList.add('absolute', '-bottom-0.5', 'right-0', 'inline-flex', 'items-center', 'gap-1');
            let timeText = document.createElement('div');
            timeText.classList.add('text-xs', 'text-[#667781]', 'dark:text-[#99beb7]');
            timeText.textContent = messageTime;
            timeContainer.appendChild(timeText);

            let invisibleSpan = document.createElement('span');
            invisibleSpan.classList.add('invisible', 'inline-flex', 'w-10', 'bg-white', 'p-2');
            invisibleSpan.setAttribute('aria-hidden', 'true');
